...
import { fetchProducts } from '@/lib/students'
    ...
    const loadProducts = async () => {
      try {
        ...
        if (apiProducts.length > 0) {
          ...
          console.warn('API returned no students, using dummy data as fallback')
        }
      } catch (err) {
      ...
      <div className='mb-2 flex items-center justify-between space-y-2'>
        <div>
          ...
          <p className='text-muted-foreground'>
            {loading ? 'Loading students...' : `Here's a list of your students (${productsData.length} items)`}
          </p>
          {error && (
